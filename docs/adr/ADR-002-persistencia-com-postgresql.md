...
Conforme previsto na [ADR-001](./ADR-001-armazenar-restaurantes-em-memoria.md), o armazenamento em memória foi adotado para a fase de MVP. Com a validação do fluxo de negócio concluída, o serviço EasyFood precisa agora de persistência real para suportar:
...
- O ambiente de desenvolvimento utiliza PostgreSQL, com a string de conexão configurada via variável de ambiente (`DATABASE_URL`).
- A biblioteca de migrations adotada foi o **Prisma** (`prisma migrate`), responsável tanto pelo versionamento do schema quanto pela geração do client de acesso ao banco.
- A migração dos dados do array em memória para o banco foi feita de forma incremental, endpoint por endpoint — ver [ADR-003](./ADR-003-arquitetura-em-camadas.md) para a evolução da organização do código nesse processo.
